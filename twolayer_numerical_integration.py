...
ax[0].plot(x[data_plot], zeta1[data_plot], 'b-', x[data_plot], zeta2[data_plot], 'r-')
ax[0].plot(x[data_plot], zeta1_g[data_plot], 'b--', x[data_plot], zeta2_g[data_plot], 'r--')
ax[1].plot(x[data_plot], zeta1[data_plot], 'b-', x[data_plot], zeta2[data_plot], 'r-')
ax[1].plot(x_pv[1:-1], zeta1_pv, 'b--', x_pv[1:-1], zeta2_pv, 'r--')
ax[0].grid(); ax[1].grid()
ax[0].ticklabel_format(axis = 'both', style = 'sci', scilimits = (-2, 2))
ax[1].ticklabel_format(axis = 'both', style = 'sci', scilimits = (-2, 2))
ax[0].set_ylim([-1e-6, 1e-6]); ax[1].set_ylim([-1e-6, 1e-6])
ax[0].set_xlim([-1e4, 1e4]); ax[1].set_xlim([-1e4, 1e4])
ax[0].set_xlabel('x (km)'); ax[0].set_ylabel('$\zeta$ (s$^{-1}$)')
ax[1].set_xlabel('x (km)');# ax[1].set_ylabel('$\zeta$ (s$^{-1}$)')
n_hours = int(t_max / 3600.)
fig.suptitle('Numerical Integration Comparison (' +str(n_hours) + (' hours)' if n_hours != 1 else ' hour)'), fontweight = 'bold', fontsize = 22)
ax[0].set_title('with Geostrophic Vorticity')
ax[1].set_title('with PV Inversion') 
ax[0].legend([r'$\zeta_1$', r'$\zeta_2$', r'$\zeta_{g1}$', r'$\zeta_{g2}$'])
#, loc = (1.01, 0.56)), #
ax[1].legend([r'$\zeta_1$', r'$\zeta_2$', r'$\zeta_{PVI1}$', r'$\zeta_{PVI2}$'])
